...
/** Producers with full profile data available in the demo. */
export const AVAILABLE_PRODUCER_NAMES = new Set([
  "María Quispe",
  "Rosa Mamani",
  "Pedro Vargas",
  "José Mendoza",
...
// ─── Per-producer generated data ───

const MONTHS_LABELS = ["Oct", "Nov", "Dic", "Ene", "Feb", "Mar"]

/** Deterministic pseudo-random from producer name, so data is stable across renders. */
...
export function getProducerIncome(producer: Producer): MonthlyIncome[] {
  const base = producer.monthlyIncome
  return MONTHS_LABELS.map((month, i) => {
    // Tier A: steady upward. Tier B: flat/wobbly. Tier C: erratic.
    const s = seed(producer.name, i) % 100
    let variation: number
    if (producer.tier === "A") {
      variation = -0.15 + (i / 5) * 0.2 + (s / 100) * 0.08
    } else if (producer.tier === "B") {
...
export function getProducerSources(producer: Producer): IncomeSource[] {
  return CROP_SOURCES[producer.crop] ?? [
    { source: producer.crop, pct: 100, color: "var(--color-chart-2)" },
  ]
}
...
export function getProducerTrustChecks(producer: Producer): TrustCheck[] {
  if (producer.tier === "A") {
    return TRUST_LABELS.map((label) => ({ label, passed: true }))
  }
  if (producer.tier === "B") {
    // One check borderline-fails for some B producers
    const failIdx = seed(producer.name, 99) % 4
    return TRUST_LABELS.map((label, i) => ({
      label,
      passed: i !== failIdx || producer.trustScore > 6.5,
    }))
  }
...
export function getProducerFlag(producer: Producer): FlaggedTransaction | null {
  if (producer.tier !== "C") return null
  const crop = producer.crop.split("/")[0]
  if (producer.name === "José Mendoza") {
...
export function getLoanRange(producer: Producer): { min: number; max: number } | null {
  if (producer.tier === "C") return null
  const base = producer.monthlyIncome
  if (producer.tier === "A") return { min: Math.round(base * 2), max: Math.round(base * 5) }
  return { min: Math.round(base * 1), max: Math.round(base * 2.5) }
}

// ─── Additional per-producer data (cooperative, hectares, stability, data sources) ───
...
export function getProducerCooperative(producer: Producer): string {
  return COOPERATIVES[producer.region] ?? "Cooperativa local"
}

export function getProducerHectares(producer: Producer): number {
  const base = producer.tier === "A" ? 3 : producer.tier === "B" ? 1.5 : 0.8
  const variation = (seed(producer.name, 77) % 30) / 10
  return Math.round((base + variation) * 10) / 10
}

export function getProducerStability(producer: Producer): number {
  if (producer.tier === "A") return 75 + (seed(producer.name, 50) % 20)
  if (producer.tier === "B") return 55 + (seed(producer.name, 50) % 20)
  return 30 + (seed(producer.name, 50) % 25)
}

export function getProducerDataSources(producer: Producer): DataSource[] {
  const coop = COOPERATIVES[producer.region]
  const sources: DataSource[] = [
    { source: coop ?? "Cooperativa local", verified: producer.tier !== "C" },
    { source: "Registro de ventas en acopio", verified: producer.months >= 4 },
    { source: "Historial de transacciones Chacra", verified: true },
  ]
  if (producer.tier === "A" && producer.months >= 6) {
